function userInformationHTML(user) {
    return `<h2>${user.name}
            ...
            <div class="gh-content">
                <div class="gh-avatar">
                    <a href="${user.html_url}" taget="_blank"><img src="${user.avatar_url}" width="80" height="80" alt="${user.login}"/></a>
                </div>
...
function fetchGitHubInformation(event) {
    ...
    <img src="../assets/css/103.gif" alt="loading" />
    </div>`);
    ...
    $.when(
        ...
        function(firstResponese, secondResponse) {
            var userData = firstResponese[0];
        ...
        function(errorResponse) {
            if (errorResponse.status === 404) {
                ...
                $("#gh-user-data").html(`<h4>Too many requests, please whait until ${resetTime.toLocaleTimeString()}</h4>`);
            } else {
